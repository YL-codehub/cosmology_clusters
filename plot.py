...
def plotHMFatZ(z):
    ...
    Y = np.ravel([temp.HMF(np.power(10,M),z)*(np.pi/180)**2 for M in X])
    plt.plot(X,Y)
...
def plotHMFatM(M):
    ...
    temp = cosmo.Cosmology()
    Y = np.ravel([temp.HMF(M,z)*(np.pi/180)**2 for z in X])
    plt.plot(X,Y)
...
def plotHMF3D():
    # X = M in solar Masses
    X = np.linspace(14,16,21)
    Y = np.linspace(0,5,51)
    temp = cosmo.Cosmology()
    Z = np.array([[temp.HMF(np.power(10,M),z)*(np.pi/180)**2 for M in X] for z in Y])
    X, Y = np.meshgrid(X, Y)
