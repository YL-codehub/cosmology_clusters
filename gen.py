...
#==================================
    ...
    ## Save calculus
    delta_new.tofile('boxnc'+str(int(nc))+'dx'+str(int(dx)))
...
# Reference correlation
    ...
    np.savetxt('binsCorr3',beginbins[:-1]+0.5*np.max(X)/nbins)
    np.savetxt('Corr3',np.ravel(bins))
    np.savetxt('stdCorr3',np.ravel(stdbins))
...
#==================================
    ...
    #computed
    x = np.array(np.loadtxt('binsCorr3'))
    y = np.array(np.loadtxt('Corr3'))
    y = 1+y
    # plt.semilogx()
    # plt.semilogy()
    std = np.loadtxt('stdCorr3')
